        ...
        try:
            ...
            async with aiosqlite.connect(file) as in_db:
                ...
                async for r in rows:
                    ...
                    if keep_up:
                        print(
                            f"\rheight {height} {(time.monotonic() - batch_start_time)/len(block_batch):0.2f} s/blocks   ",
                            end="",
                        )
                        batch_start_time = time.monotonic()
                    else:
                        print(f"\rheight {height} {counter/(time.monotonic() - batch_start_time):0.2f} blocks/s   ", end="")
                    block_batch = []
                ...
                logger.warning(f"duration: {end_time - start_time:0.2f}")
                logger.warning(f"worst time-per-block: {worst_batch_time_per_block}")
